...
print("2. Extracting Management Group 1 (Approx 106k rows)...")
train_g1 = train_df[train_df['management_group_id'] == 1].copy()
eval_g1 = eval_df[eval_df['management_group_id'] == 1].copy()
print(f"   Train rows: {len(train_g1):,}")
print(f"   Eval rows:  {len(eval_g1):,}")
...
def train_eval(features, name):
    ...
    X_train = train_g1[features].copy()
    y_train = train_g1['sale_amount']
    X_eval = eval_g1[features].copy()
    y_eval = eval_g1['sale_amount']
...
print("\n" + "="*60)
print(" RIGOROUS ABLATION STUDY RESULTS (106k ROWS, 1500 TREES, LR=0.03)")
print("="*60)
